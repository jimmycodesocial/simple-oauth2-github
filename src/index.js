  ...
  */
  create: (config) => {
    ...
    requiredArgs.forEach((arg) => {
      if (!args[arg.name]) {
        throw new Error(`'SimpleOAuth2Github requires a [${arg.name}]'`);
      }
      // eslint-disable-next-line valid-typeof
      else if (typeof args[arg.name] !== arg.type) {
        // eslint-disable-next-line max-len
        throw new Error(`SimpleOAuth2Github expects [${arg.name}] to be a [${arg.type}] but it was a [${typeof args[arg.name]}]'`);
      }
    });
    ...
    if (args.scope && (typeof args.scope !== 'string' && !Array.isArray(args.scope))) {
      // eslint-disable-next-line max-len
      throw new Error(`SimpleOAuth2Github expects [scope] to be a [array or string] but it was a [${typeof args.scope}]`);
    }
    ...
    const oauth2 = simpleOAuth2.create({
      ...
      auth: {
        authorizeHost: args.authorizeHost || 'https://github.com',
        authorizePath: args.authorizePath || '/login/oauth/authorize',
        tokenHost: args.tokenHost || 'https://github.com',
        tokenPath: args.tokenPath || '/login/oauth/access_token'
      }
    });
    ...
    const authorize = (req, res) => {
      const options = Object.assign(
        ...
        args.authorizeOptions || {},
        {
          ...
          scope: args.scope || ['read:user', 'read:email'],
          state: args.state || ''
        }
      );
